...
int set_option(struct packet_data *sp_pd, const char *option,
	...
	else if (!pgen_strcmp(option, "IPV6_EXT_HDR")) {
		errno = 0;
		sp_pd->ipv6_ext_hdr = strtol(value, NULL, 10);
		if (errno) {
			fprintf(stderr, "Invalid IPv6 extention header\n");
			goto err;
		}
	}
...
int parse_conf_file(struct packet_data *sp_pd) {
	...
	/* IPv6 Extention headers */
		...
		int i = 0; /// Need to be pushed at first
		struct ipv6_extention *ipv6_ext = NULL;
		for (i = 0; i < sp_pd->ipv6_ext_hdr; i++) {
			while(ipv6_ext)
				ipv6_ext = ipv6_ext->next;

			ipv6_ext = malloc(sizeof(struct ipv6_extention));
			if (ipv6_ext == NULL) {
				perror("mallco");
				goto err;
			}
			/* head */
			if (!sp_pd->ipv6_ext)
				sp_pd->ipv6_ext = ipv6_ext;

			///Kept inside a function
			fscanf(fp, "%s", line);
			/* Ignore comment lines */
			if (line[0] == '#') {
				char c;
			...
			*c = '\0';
			...
			if (!pgen_strcmp(option, "EXT_HDR")) {
				errno = 0;
				ipv6_ext->hdr_type = strtol(value, NULL, 10);
				if (errno) {
					fprintf(stderr, "Invalid extention header type\n");
					goto err;
				}
			}

			if (ipv6_ext->hdr_type == 0) {
				///do nothing now. put the follwoing portion into here
			}
			...
			*c = '\0';
			...
			if (!pgen_strcmp(option, "HOP-BY-HOP_OP")) {
				errno = 0;
				ipv6_ext->data = malloc(sizeof(int)); ///wrong approach
				int t = strtol(value, NULL, 10);
				if (errno) {
					fprintf(stderr, "Invalid hop-by-hop option\n");
					goto err;
				}
				memcpy(ipv6_ext->data, &t, 4);
			}
		}
	}
